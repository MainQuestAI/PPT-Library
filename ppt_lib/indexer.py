...
def index_file(path: Path, settings: Settings, *, full: bool = False, refresh_versions: bool = True) -> IndexResult:
    ...
    try:
        ...
        if should_skip_file(path, existing, full=full):
        ...
        for slide_index in range(total):
            ...
            pending_slides.append(
                _PendingSlide(
                    ...
                    metadata_json=_metadata_with_embedding(vision.metadata, settings, provider.dimensions),
                )
            )
...
def should_skip_file(path: Path, existing: dict[str, object] | None, full: bool) -> bool:
...
def _metadata_with_embedding(metadata: dict[str, object], settings: Settings, dimensions: int) -> dict[str, object]:
    enriched = dict(metadata)
    enriched["embedding"] = {
        "provider": settings.embedding_provider,
        "model": settings.lmstudio_embedding_model if settings.embedding_provider == "lmstudio" else settings.embedding_model,
