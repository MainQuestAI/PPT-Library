...
from ppt_lib.diagnostics import check_api_key, probe_lmstudio, probe_mmx, run_diagnostics
...
def test_ollama_detected_with_vision_model(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "ppt_lib.diagnostics.probe_mmx",
        lambda command="mmx", timeout=30.0: ("warning", "not checked", {}),
    )
    settings = load_settings({"home_dir": tmp_path}, config_path=tmp_path / "config.yml")
...
def test_cloud_vision_without_key_is_not_masked_by_lmstudio(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr("ppt_lib.diagnostics.shutil.which", lambda name: "/usr/bin/soffice")
    ...
    settings = load_settings(
        {
            ...
            "vision_api_key": None,
            "vision_max_slides_per_file": None,
        },
        config_path=tmp_path / "config.yml",
    )
    ...
    assert chains["vision"]["provider"] == "cloud"
    assert chains["vision"]["status"] == "skipped"


def test_mmx_probe_checks_auth_and_quota(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[list[str]] = []

    def fake_run(args, **kwargs):
        calls.append(args)
        if args[1:3] == ["auth", "status"]:
            return type("Completed", (), {"returncode": 0, "stdout": '{"method":"api-key","key":"sk-c...xxxx"}', "stderr": ""})()
        return type("Completed", (), {"returncode": 0, "stdout": '{"model_remains":[]}', "stderr": ""})()

    monkeypatch.setattr("ppt_lib.diagnostics.shutil.which", lambda command: "/usr/local/bin/mmx")
    monkeypatch.setattr("ppt_lib.diagnostics.subprocess.run", fake_run)

    status, message, details = probe_mmx("mmx")

    assert status == "ok"
    assert message == "mmx auth and quota probe passed"
    assert calls[0][1:3] == ["auth", "status"]
    assert calls[1][1:3] == ["quota", "show"]
    assert details["auth"] == {"method": "api-key", "key": "sk-c...xxxx"}


def test_mmx_vision_chain_is_selected_when_configured(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr("ppt_lib.diagnostics.shutil.which", lambda name: "/usr/bin/soffice" if name == "soffice" else "/usr/local/bin/mmx")
    monkeypatch.setattr("ppt_lib.diagnostics.probe_ollama", lambda base_url, timeout=2.0: ("warning", "not running", {}))
    monkeypatch.setattr(
        "ppt_lib.diagnostics.probe_lmstudio",
        lambda base_url, timeout=2.0, vision_model=None: ("ok", "configured vision model image probe passed", {}),
    ...
    assert payload["can_use_vision"] is True
    assert payload["recommended_chain"] == ["mmx", "text_extraction"]
    assert chains["vision"]["provider"] == "mmx"
    assert chains["vision"]["status"] == "ok"
